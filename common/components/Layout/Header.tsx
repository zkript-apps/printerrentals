    ...
    <WidthWrapper width="medium">
      <nav className="flex justify-between items-center h-16">
        ...
        <div className="pr-5 md:hidden" onClick={handleMenuClick}>
          <div
            className={`transition-transform duration-500 ease-in-out transform ${isMenuOpen ? "rotate-90" : ""
              }`}
          >
        ...
        <div
          className={`transition-all duration-200 ease-in-out transform origin-top ${isMenuOpen ? "scale-y-100" : "scale-y-0"
                  ...
                  >
                    <div className="w-full">
                      <Typography
                        variant="h5"
                        fontWeight="semibold"
                        className={`text-blue-950 hover:text-orange-400 transition-colors duration-200 ${link.isActive ? "text-orange-400" : ""
                          }`}
                      >
                        {link.name}
                      </Typography>
                    ...
                    {link.subMenu.map((subLink, index) => (
                      <Link href={subLink.link} key={subLink.name}>
                        <Typography
                          variant="h5"
                          fontWeight="semibold"
                          className={`text-blue-950 hover:text-blue-900 transition-colors duration-200 ${index < link.subMenu.length - 1 ? "mb-5" : ""
                            }`}
                        >
                          {subLink.name}
                        </Typography>
